...
## 7. Monthly Execution Health Boundary

- 现役模型是 `FINANCIAL_HEALTH_MODEL` version 1，定义于 `scripts/app-calculations.js`。
- 它根据计划收入执行、消费预算、月度自由现金和资产判断基线生成提示分数。
- 权重和阈值由代码中的模型对象统一提供，评分弹窗不得另写一套数字。
- 缺少净值快照导致资产基线不完整时会降低评分。
- 该分数只描述月度记录、预算和现金流执行，不等于综合财务风险。
- 负债率、应急金覆盖、流动性、资产集中度和投资回撤需要独立定义后，才能形成结构性风险模型。
- 修改权重或阈值时必须提升模型版本，并补充边界与代表场景测试。
