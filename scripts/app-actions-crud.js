...
function removeRecord(type, id) {
  var map = { income: "incomes", expense: "expenses", investment: "investments", snapshot: "snapshots", account: "accounts", assetItem: "assetItems" };
  var key = map[type];
  if (!key) return;
  appConfirm("确认删除", "确定删除这条数据吗？", "删除", "取消").then(function (ok) {
    if (!ok) return;
    if (type === "account") {
      var used = state.expenses.some(function (x) { return x.accountId === id; }) || state.investments.some(function (x) { return x.accountId === id; }) || state.snapshots.some(function (x) { return x.accountId === id; });
      if (used) {
        appConfirm("账户仍有关联记录", "这个账户已有历史记录。删除后相关记录会显示为“已删除账户”。是否继续？", "继续删除", "取消").then(function (confirmed) {
          if (!confirmed) return;
...
function editRecord(type, id) {
  if (type === "income") {
    ...
    byId("incomeMonth").value = income.month;
  ...
  if (type === "expense") {
    ...
    byId("expenseMonth").value = expense.month;
  ...
  if (type === "investment") {
    ...
    byId("investmentMonth").value = inv.month;
    byId("investmentAccount").value = inv.accountId;
    byId("investmentType").value = inv.type;
  ...
  if (type === "snapshot") {
    ...
    byId("snapshotMonth").value = snap.month;
